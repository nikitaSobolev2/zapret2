...
[Скрипты запуска](#скрипты-запуска) и [инсталлятор](#инсталлятор) поддерживают установку на любые классические дистрибутивы Linux с systemd или openrc, из прошивок - на openwrt, а также на macOS.
Если система не удовлетворяет указанным требованиям - возможна самостоятельная "доприкрутка" к системе.

nfqws2 не может работать в macOS по причине отсутствия подходящего средства перехвата и управления пакетами. Стандартное для BSD средство ipdivert было убрано из ядра производителем, а в PF нет divert-packet.
...
Поэтому macOS поддерживается через другой демон - **tpws**, прозрачный TCP прокси. Он работает не с отдельными пакетами, а с данными уже установленного соединения.
Трафик заворачивается в него правилами PF `rdr` и `route-to`, которые лежат в анкорах `zapret2`, `zapret2-v4` и `zapret2-v6`, автозапуск делается через launchd plist. См. `init.d/macos` и `common/pf.sh`.
Бинарники собираются командой `make mac` - получаются universal (arm64 + x86_64) `tpws`, `ip2net` и `mdig`. `install_easy.sh` определяет Darwin и настраивает все сам, стратегия задается в `TPWS_ENABLE` и `TPWS_OPT` в [config](#файл-config).

У схемы на PF есть два следствия, которые надо учитывать. tpws нужен `/dev/pf`, чтобы узнать исходного получателя перенаправленного соединения, поэтому он работает под root, а трафик root исключен из перенаправления, чтобы не завернуть его обратно в tpws - трафик процессов, работающих под root, не обрабатывается.
Доступны стратегии прозрачного прокси, а не Lua атаки nfqws2, и [blockcheck2](#blockcheck2) в macOS только определяет блокировки, но не подбирает стратегию обхода.
