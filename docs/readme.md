...
традиционные Linux-системы, FreeBSD, OpenBSD, Windows. В macOS перехват пакетов невозможен, поэтому там работает
прозрачный прокси tpws с заворотом трафика через PF - см. [мануал](manual.md).
